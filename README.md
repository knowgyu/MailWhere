...
핵심 방향은 “메일을 대신 처리하는 agent”가 아닙니다. Outlook 원본은 그대로 두고, MailWhere 안에서만 업무 후보를 확인하고 `나중에`, `수정`, `보관`으로 정리합니다.
...
- **업무 보드 중심**: 보드는 전체/오늘/7일 내/30일 내/기한 미정 필터와 `내가 할 일`/`기다리는 중` 2열 카드로 활성 항목을 보여줍니다.
- **간단한 카드 액션**: `열기`, `나중에`, `수정`, `보관`을 기본 액션으로 둡니다. 기한은 카드의 기한 버튼에서 빠르게 바꿉니다.
...
- 낮은 확신/LLM 실패 후보는 기본 업무 보드에 섞지 않고 검토 후보에서 처리
- 업무 카드 더블클릭 또는 `수정`으로 제목, 간단한 유형, 기한 수정
...
테스트 중 로컬 업무/검토 데이터를 지우고 다시 시작하려면 앱의 **문제 해결 → 업무 데이터 초기화**를 누르거나 아래 스크립트를 실행합니다. 기본은 `%LOCALAPPDATA%\\MailWhere\\followups.sqlite*`만 삭제하고 설정은 유지합니다.
