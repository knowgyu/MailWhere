...
- Task cards use `열기`, `나중에`, `수정`, `보관` as primary actions.
...
- Bounded edit dialog supports AI-derived title, simple visible category, and due date edits.
...
- 카드 기본 액션은 `열기`, `나중에`, `수정`, `보관`으로 정리한다. 기한은 카드의 기한 버튼에서 처리한다.
...
- 카드 액션: Outlook 열기, 나중에 보기, 수정, 보관, 기한 지정.
