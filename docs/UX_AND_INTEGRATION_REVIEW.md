...
2. **검토함 액션**: review candidate 표시는 가능하지만 승인/삭제/할 일 전환 버튼이 아직 없다. false positive control을 위해 0.2 최우선.
3. **자동 watcher**: 0.1.0은 startup auto scan gate와 reminder timer 중심이다. Outlook event subscription 또는 polling은 smoke gate 이후에만 켠다.
