...
4. **LLM 실패 허용**: endpoint가 없거나 JSON이 깨져도 검토 후보에 보관하고, endpoint 복구 후 다시 분석할 수 있어야 함.
...
## v0.4.1 Windows smoke에서 남은 UX TODO

아래 항목은 v0.4.1 릴리즈 이후 실제 화면 확인에서 확인된 다음 패치 후보다.

1. **메인 헤더 버튼 정렬**: 버튼 묶음이 오른쪽에 어색하게 떠 있고 focus rectangle이 기본 WPF처럼 보여 polish가 부족하다. Primary `지금 메일 확인`은 오른쪽 끝에 안정적으로 두고, `검토 후보`/`설정`/`+ 직접 추가`는 같은 버튼 체계로 정돈한다.
2. **필터 순서 변경**: 업무 보드 필터는 `오늘` / `이번 주` / `전체` / `날짜 없음` 순서가 더 자연스럽다.
3. **개발자 도구 가시성 복구**: 개발자 도구를 daily header에 다시 올리지는 않되, 설정 안에서는 기존 샘플 데이터/알림/필터/오늘 표시 reset류 도구가 명확히 보여야 한다.
4. **검토 후보 단축키 단순화**: `Alt+A/S/I` 대신 `등록(Y)`, `나중에(S)`, `무시(I)`로 바꾸고, 행 버튼에 focus를 주지 않아도 창 단위 단축키로 동작하게 한다.
5. **검토 후보 버튼 순서**: 사용자는 먼저 원본을 보고 판단하는 경우가 많으므로 `원본 열기`를 `등록`보다 앞에 둔다.
6. **검토 후보 창 톤 통일**: 현재 검토 후보 창은 메인 보드보다 raw WPF 느낌이 강하다. 공유 버튼 스타일, 카드 radius/padding, header spacing, muted text tone을 main window와 맞춘다.
7. **샘플 데이터 품질**: 같은 제목/발신자/시간이 반복되면 실제 중복처럼 보여 제품 신뢰도가 떨어진다. 개발자 샘플은 의도적으로 다양하게 만들거나 “샘플 반복 생성” 표시를 둔다.
8. **용어 정리**: `검토 후보`의 `후보`가 사용자에게 애매하다. 사용자-facing 명칭은 `확인 필요` 또는 `확인할 항목` 쪽으로 바꾸고, `AI 실패 후보 다시 분석`은 `AI 분석 다시 시도`/`AI로 다시 확인`처럼 실패와 후보라는 단어를 줄인다.

내가 추가로 보는 개선 후보:

- **Windows visual smoke를 릴리즈 체크리스트화**: main board, review candidates, settings, edit dialog, toast를 같은 배율에서 캡처해 release 전에 확인한다.
- **메인 보드 키보드 사용성**: Enter로 열기, S로 나중에, Archive 키 또는 Delete/Backspace confirmation 등은 나중에 추가 가치가 크다.
- **검토 후보 bulk 처리**: 후보가 여러 개 쌓였을 때 한 행씩 누르는 흐름은 금방 피곤해진다. 다중 선택/일괄 나중에/일괄 무시를 후속으로 둔다.
...
2. **검토 후보 액션**: 단일 선택 등록/무시는 생겼지만, 다중 선택/일괄 처리/마감 수정은 아직 없다.
