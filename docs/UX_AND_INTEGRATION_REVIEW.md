# UX / Integration Review for 0.1.0

Date: 2026-05-15

## 핵심 제품 정의

Outlook AI Secretary의 핵심은 “메일 앱을 한 번 더 열어보게 만드는 도구”가 아니라, Windows에 조용히 떠 있으면서 메일 기반 후속 조치와 마감 리스크를 먼저 알려주는 개인 업무 비서다. 그래서 0.1.0 UX 판단 기준은 기능 수보다 아래 4개다.
...
- Tray-first notification: 앱을 열어보지 않아도 scan summary/reminder/error를 보여줌.
- 검토함 표시: 자동 등록하기 애매한 후보를 앱 안에서 확인 가능하게 함.
- 오늘의 업무 보드: 기본 08:00 또는 앱 시작 후 다음 정시에 오늘/지남, 7일 내, 마감 없음, 확인 필요 후보를 보드 창으로 표시.
- 확인 필요 팝업: 새 review candidate는 우측 하단 lightweight popup의 버튼으로 처리하고, 팝업을 클릭해 초점을 둔 경우 Ctrl+Y 등록 / Ctrl+N 무시 단축키도 지원.
- Reminder timer: 앱이 켜져 있는 동안 30분마다 due reminder 후보를 재검토.
- Automatic watcher smoke gate 통과 후에는 15분마다 보수적으로 read-only scan을 수행해 새 review candidate를 띄움.
- LLM 설정 UI: Disabled/Ollama/OpenAI-compatible 선택, endpoint/model, API key 환경 변수명 지원.
...
2. **검토함 액션**: 단일 선택 등록/무시와 popup 단축키는 생겼지만, 다중 선택/일괄 처리/마감 수정은 아직 없다.
...
5. **LLM 품질 관측**: confidence, fallback reason, invalid JSON count가 UI에 더 드러나야 endpoint 교체 시 품질을 판단할 수 있다.

## OfficeWhere 연결성

- 단기: 메일 action item에서 추출한 키워드를 OfficeWhere search query로 넘기는 “검색 핸드오프”가 가장 안전하다.
- 중기: Outlook AI Secretary가 만든 task/review candidate를 JSONL 또는 SQLite view로 export하고, OfficeWhere가 이를 read-only source로 색인한다.
